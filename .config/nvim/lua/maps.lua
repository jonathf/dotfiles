...
vim.keymap.set("n", "g:", ":Gcd<cr>:", { desc = "cmd-root", noremap = true })
vim.keymap.set(
	"n",
	":",
	":silent! exec ':cd '.fnamemodify(expand('%'), ':h')<cr>:",
	{ desc = "cmd-local", noremap = true }
)
vim.keymap.set("n", "<space>r", ":IncRename ", { desc = "inc-rename" })
